...
void *cpu_monitoring_loop(void *data)
{
	...
	dpdk_start = monotonic_time();
	
	while(1)
	{
		end = monotonic_time();
		if(end - start > (long long)ONE_SEC)
		{
			print_cur_stat(r_size, u_str[unit]);
			start = end;
			gpu_recv = get_rx_cnt();
			cpu_recv = 0;
			copy_cnt = 0;
...
{
	long long cur_time = monotonic_time();
	long long min = (cur_time - dpdk_start) / ((long long)ONE_SEC * 60);
	long long sec = ((cur_time - dpdk_start) / (ONE_SEC)) % 60;
	system("clear");
	gpu_recv = get_rx_cnt();
	gpu_recv /= MEGA;
	cpu_recv /= MEGA;	
...
static void rx_loop(uint8_t lid)
{
	...
	unsigned int i, j;
#if 0
	long long gpu_recv = 0;
	long long cpu_recv = 0;
	int copy_cnt = 0;
#endif
